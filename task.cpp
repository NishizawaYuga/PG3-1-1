	...
	if (newNum < 1000)
	{
		...
		while (true) {
			...
			if (instructionsNum == 0) {
				...
				cout << "C³‚µ‚½‚¢‰ÓŠ‚ª‚ ‚è‚Ü‚µ‚½‚çuƒ^ƒXƒNƒf[ƒ^•ÏXv‚©‚ç" << endl;
				cout << "‚¨Šè‚¢‚µ‚Ü‚·" << endl;
				break;
			}
			else if (instructionsNum == 1) {
...
void Task::ViewList() {
	for (int i = 0; i < dataNum; i++) {
		//ƒf[ƒ^‚È‚µ‚ÍÈ‚­
		if (taskData[i].dataRetention) {
			cout << "yIDz" << taskData[i].id << "\ty‘è–¼z" <<
				taskData[i].name << "\ty—Dæ“xz" <<
				ViewPriority(taskData[i].priority) << "\tyƒXƒe[ƒ^ƒXz" <<
